...
2. configure Vite to use the `@qwik.dev/react` plugin:
   ```ts
   // vite.config.ts
...
The `@qwik.dev/react` package exports the `qwikify$()` function that lets you convert React components into Qwik components, that you can use across your application.
